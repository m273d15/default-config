...
angular.module('bahmni.common.displaycontrol.custom')
    ...
    var link = function ($scope) {
        ...
        var convertUTCtoLocal = function (date, time) {
            date = date.split("/");
            ...
            var startTime = Bahmni.Common.Util.DateUtil.formatTime(dateTimeStart.getTime());
            var endTime = Bahmni.Common.Util.DateUtil.formatTime(dateTimeEnd.getTime());
            date = String(dateTimeStart.getDate()).padStart(2, '0') + "/" + String(dateTimeStart.getMonth()).padStart(2, '0') + "/" + String(dateTimeStart.getFullYear());
        ...
        $q.all([getUpcomingAppointments(), getPastAppointments()]).then(function (response) {
            ...
            for (var i=0; i<$scope.upcomingAppointments.length; i++) {
                ...
                const [date, timeSlot] = convertUTCtoLocal($scope.upcomingAppointments[i].DASHBOARD_APPOINTMENTS_DATE_KEY, $scope.upcomingAppointments[i].DASHBOARD_APPOINTMENTS_SLOT_KEY);
